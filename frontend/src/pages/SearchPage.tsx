...
export default function SearchPage() {
  ...
  const updateQuery = (value: string) => setParams(value ? { q: value, type } : {}, { replace: true });
  ...
  const results = useMemo(() => {
    ...
    if (sort === 'title') items.sort((a,b) => a.title.localeCompare(b.title, 'pt-BR'));
    if (sort === 'recent') items.sort((a,b) => (b.release_date || '').localeCompare(a.release_date || ''));
    return items;
  }, [movies, rating, sort]);
