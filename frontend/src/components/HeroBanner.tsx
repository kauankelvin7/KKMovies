...
export function HeroBanner({ movies, loading }: { movies: Movie[]; loading?: boolean }) {
  ...
  useEffect(() => {
    if (paused || hovered || picks.length < 2 || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    const timer = setInterval(() => { if (!document.hidden) setIndex(value => (value + 1) % picks.length); }, 8000);
    return () => clearInterval(timer);
  }, [paused, hovered, picks.length]);
  ...
  useEffect(() => { if (movie) setInList(watchlistService.isInList(movie.id, movie.media_type === 'tv' ? 'tv' : 'movie')); }, [movie]);
  ...
  return <section className="cinema-hero" aria-label="Destaques do catálogo" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onFocusCapture={() => setHovered(true)} onBlurCapture={event => { if (!event.currentTarget.contains(event.relatedTarget)) setHovered(false); }} onTouchStart={event => { touch.current = event.touches[0].clientX; }} onTouchEnd={event => { if (touch.current === null || picks.length < 2) return; const delta = event.changedTouches[0].clientX - touch.current; if (Math.abs(delta) > 55) { setIndex(value => (value + (delta < 0 ? 1 : picks.length - 1)) % picks.length); setPaused(true); } touch.current = null; }}>
    ...
    {picks.length > 1 && <button className="hero-pause absolute right-6 top-24 z-10" onClick={() => setPaused(value => !value)} aria-pressed={paused}>{paused ? 'Retomar slides' : 'Pausar slides'}</button>}
